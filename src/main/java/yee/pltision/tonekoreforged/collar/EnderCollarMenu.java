...
public class EnderCollarMenu extends AbstractContainerMenu implements CollarMenu {
    ...
    protected EnderCollarMenu(@Nullable MenuType<?> p_38851_, int p_38852_) {
        super(p_38851_, p_38852_);
    }
    public EnderCollarMenu(int p_38852_, Inventory inventory) {
        super(ToNeko.BASIC_COLLAR_MENU.get(), p_38852_);
        this.container=new SimpleContainer(SLOT_SIZE);
        initSlots(inventory);
    }
    public EnderCollarMenu(int p_38852_, Inventory inventory, @NotNull CollarState state, @Nullable Function<Player,Boolean> validChecker) {
        super(ToNeko.BASIC_COLLAR_MENU.get(), p_38852_);
        this.state=state;
//        this.item=item;
    ...
    public EnderCollarMenu(int p_38852_, Inventory inventory, @NotNull CollarState state, ItemStack item) {
    ...
    public void initSlots(Inventory inventory){
        ...
        for(int i = 0; i < 3; ++i) {
            for(int j = 0; j < 9; ++j) {
                this.addSlot(new Slot(inventory, j + i * 9 + 9, 9 + j * 18, 66 + i * 18+18));
            }
        }

        for(int i = 0; i < 9; ++i) {
            this.addSlot(new Slot(inventory, i, 9 + i * 18, 124+18));
        }
